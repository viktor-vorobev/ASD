...
public:
    SortedTableOnRB() = default;
    ~SortedTableOnRB() override = default;
    ...
    std::vector<TKey> getKeys() const override {
        ...
        keys.reserve(_tree.size()); // Оптимизация: выделяем память одним куском
    ...
    std::vector<TVal> getValues() const override {
        ...
        vals.reserve(_tree.size()); // Оптимизация: выделяем память одним куском
    ...
    void deserialize(const std::string& data) override {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            // Безопасное изолированное чтение ключа
            if (!std::getline(ss, line)) break;
            std::stringstream keyStream(line);
            keyStream >> key;

            // Безопасное изолированное чтение значения
            if (!std::getline(ss, line)) break;
